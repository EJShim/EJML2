        ...
        if(length%2 === 1){
          ...
          for(var k=0 ; k<length ; k+=2){
            ...
            for(var m=0 ; m<iter ; m++){
              ...
              var idx = this.PositionToVoxelIdx(min, voxelSize, pos);
              voxelSpace[idx.x][idx.y][idx.z-1] = 1;
            }
          }
        }
      }
...
E_Manager.prototype.PositionToVoxelIdx = function(min, voxelSize, position)
{
  ...
  idx.x = Math.round((position.x-min.x)/voxelSize - 0.5);
  idx.y = Math.round((position.y-min.y)/voxelSize - 0.5);
  idx.z = Math.round((position.z-min.z)/voxelSize - 0.5);
